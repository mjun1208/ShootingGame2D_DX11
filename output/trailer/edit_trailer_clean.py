...
WORK=ROOT/'work-clean'; WORK.mkdir(exist_ok=True)
...
# Align each popup onset 0.1 seconds into an unretimed 0.8-second cut.
...
SHOTS=[SHOTS[0]]+[(t-.1,.8,1,.8,'','',1.8) for t in POPUPS]+SHOTS[8:]
SHOTS=[(*s[:4],'','',s[6]) for s in SHOTS]

def overlap(i):
    return 0 if 2 <= i <= 8 else TRANS

def shot_source(i):
    return NEW_VIDEO if 1 <= i <= 8 else VIDEO

# Reuse only clips that never had editorial captions baked in.
for i,s in enumerate(SHOTS):
    if 1 <= i <= 8: continue
    old_i=0 if i==0 else i-1
    if not ORIGINAL_SHOTS[old_i][4]:
        cached=ROOT/'work-1x'/f'shot-{old_i:02}.mp4'
...
if __name__=='__main__':
    ...
    (ROOT/'timeline-clean.json').write_text(json.dumps(timeline,indent=2),encoding='utf8')
    ...
    # The accent peaks at each popup onset, on the same 0.8-second beat.
    hits=accents(duration,[timeline[i]['start']+.045 for i in range(1,9)]+[timeline[i]['start'] for i in [11,13,16,19,25]])
    final=ROOT/'DungeonSurvivor_Trailer_Clean.mp4'
