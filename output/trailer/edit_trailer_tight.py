...
WORK=ROOT/'work-tight'; WORK.mkdir(exist_ok=True)
...
# Keep existing shots only when all source and framing parameters match.
BASE_SHOTS=list(SHOTS)
SHOTS[0]=(8.5,1.1,1,1.1,'','',1)
for i,d in [(13,2.7),(14,2.7),(15,2.9)]:
    old=SHOTS[i];SHOTS[i]=(old[0],d,1,d,'','',1)
# Alternate ordinary enemies and bosses, cutting between completed dash passes.
DASHES=[(204.8,1.4),(140.7,1.2),(243.7,1.4),(223.5,1.0)]
SHOTS=SHOTS[:16]+[(t,d,1,d,'','',1) for t,d in DASHES]+SHOTS[19:]

def overlap(i):
    if 17 <= i <= 19:return 0
    return .3 if 1 <= i <= 9 else TRANS

# Halve the six Cthulhu shots together, preserving the reveal and live combat.
for i,d in zip(range(26,32),[2,1.4,1.3,1.9,1.3,1.2]):
    old=SHOTS[i];SHOTS[i]=(old[0],d,1,d,'','',old[6])
cached_timeline=json.loads((ROOT/'timeline-dash-short.json').read_text())
for i,s in enumerate(SHOTS):
    for row in cached_timeline:
        if s[0]==row['source_start'] and s[1]==row['source_duration'] and str(shot_source(i))==row['source']:
            cached=ROOT/'work-dash-short'/f"shot-{row['shot']:02}.mp4"
            target=WORK/f'shot-{i:02}.mp4'
            if cached.exists() and not target.exists():shutil.copyfile(cached,target)
            break
for i in range(5):
    cached=ROOT/'work-dash-short'/f'group-{i}.mp4'
...
if __name__=='__main__':
    ...
    (ROOT/'timeline-tight.json').write_text(json.dumps(timeline,indent=2),encoding='utf8')
    ...
    final=ROOT/'DungeonSurvivor_Trailer_Tight.mp4'
