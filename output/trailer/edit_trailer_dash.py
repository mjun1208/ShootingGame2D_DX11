...
WORK=ROOT/'work-dash'; WORK.mkdir(exist_ok=True)
...
DASHES=[(204.6,2.3),(140.6,1.3),(18.5,1.5),(223.4,1.4),(243.6,1.7),(294.4,1.8)]
...
for i,s in enumerate(SHOTS):
    if i==0:continue
    for j,old in enumerate(BASE_SHOTS):
        if s==old and shot_source(i)==(NEW_VIDEO if 1<=j<=8 else VIDEO):
            cached=ROOT/'work-smooth'/f'shot-{j:02}.mp4'
...
if __name__=='__main__':
    ...
    (ROOT/'timeline-dash.json').write_text(json.dumps(timeline,indent=2),encoding='utf8')
    ...
    final=ROOT/'DungeonSurvivor_Trailer_Dash.mp4'
