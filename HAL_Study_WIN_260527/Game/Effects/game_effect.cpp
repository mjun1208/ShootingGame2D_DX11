...
void cGameEffectManager::Initialize()
{
	...
	m_Definitions[static_cast<std::size_t>(GameEffectType::SmokePoof)] = { L"asset/texture/vfx/smoke-poof/smoke.png",
		                                                                   TEXTURE_INVALID_ID,
		                                                                   32,
		                                                                   32,
		                                                                   7,
	...
	m_Definitions[static_cast<std::size_t>(GameEffectType::PixelMagicHit)] = {
		L"asset/texture/vfx/codemanu/free-pixel-effects-pack/5_magickahit_spritesheet.png",
		TEXTURE_INVALID_ID,
		100,
		100,
		49,
	...
	m_Definitions[static_cast<std::size_t>(GameEffectType::DashSlashHitCut)] = {
		L"asset/texture/vfx/frostwindz/pixel-art-slashes/slash_2_color3_128.png",
		TEXTURE_INVALID_ID,
		128,
		128,
		7,
	...
	m_Definitions[static_cast<std::size_t>(GameEffectType::EnemyWarriorSlash)] = {
		L"asset/texture/vfx/tbbk/pixel-sword-slash/pixel_art_sword_slash_sprites.png",
		TEXTURE_INVALID_ID,
		64,
		47,
		9,
	...
	auto set_light = [this](GameEffectType type, const DirectX::XMFLOAT3& color, float radius, float strength)
	{
		Definition& definition = m_Definitions[static_cast<std::size_t>(type)];
		definition.LightColor = color;
		definition.LightRadius = radius;
		definition.LightStrength = strength;
	};
	set_light(GameEffectType::ElectricImpact, { 0.18f, 0.72f, 1.0f }, 230.0f, 1.05f);
	set_light(GameEffectType::WarmExplosion, { 1.0f, 0.42f, 0.10f }, 190.0f, 1.35f);
	set_light(GameEffectType::BossDeathSmallExplosion, { 1.0f, 0.34f, 0.06f }, 160.0f, 1.05f);
	set_light(GameEffectType::VoidImplosion, { 0.52f, 0.20f, 1.0f }, 210.0f, 0.90f);
	set_light(GameEffectType::SmokePoof, { 1.0f, 0.56f, 0.14f }, 190.0f, 0.50f);
	set_light(GameEffectType::PixelMagicHit, { 0.72f, 0.30f, 1.0f }, 155.0f, 0.68f);
	set_light(GameEffectType::DashSlashHitBurst, { 0.16f, 0.62f, 1.0f }, 145.0f, 0.78f);
	set_light(GameEffectType::DashSlashHitCut, { 0.18f, 0.48f, 1.0f }, 180.0f, 0.56f);
	set_light(GameEffectType::EnemyWarriorSlash, { 1.0f, 0.06f, 0.02f }, 175.0f, 0.64f);
...
{
	static constexpr float CINEMATIC_EFFECT_OPACITY = 1.0f;
	static constexpr float COMBAT_EFFECT_OPACITY = 0.68f;
	...
	for (const cEffect& effect : m_Effects)
	{
		...
		if (effect.BuildInstance(instance))
		{
			for (std::size_t type_index = 0; type_index < TYPE_COUNT; ++type_index)
			{
				...
				{
					const GameEffectType type = static_cast<GameEffectType>(type_index);
					const bool cinematic = type == GameEffectType::BossDeathSmallExplosion;
					instance.Color.w *= cinematic ? CINEMATIC_EFFECT_OPACITY : COMBAT_EFFECT_OPACITY;
	...
	for (std::size_t type_index = 0; type_index < TYPE_COUNT; ++type_index)
	{
		...
		if (!instances.empty())
		{
			SpriteInstanced_DrawUnlit(m_Definitions[type_index].TextureID, instances.data(),
			                          static_cast<int>(instances.size()));
		}
	}
}

int cGameEffectManager::AppendPointLights(SpritePointLight* lights, int light_count, int capacity) const
{
...
bool cGameEffectManager::Play(GameEffectType type, const DirectX::XMFLOAT2& position, float scale,
                              const DirectX::XMFLOAT4& color, float rotation)
{
	const Definition* definition = GetDefinition(type);
	if (!m_IsInitialized || !definition || definition->TextureID == TEXTURE_INVALID_ID || scale <= 0.0f)
	{
		return false;
	}

	const auto free_effect = std::find_if(m_Effects.begin(), m_Effects.end(),
	                                      [](const cEffect& effect)
	                                      {
		                                      return !effect.IsActive();
	                                      });
	cEffect* target = free_effect == m_Effects.end() ? nullptr : &*free_effect;
	if (!target)
	{
	...
	cEffectDesc desc{};
	desc.Position = { position.x - definition->PivotOffset.x * scale, position.y - definition->PivotOffset.y * scale };
...
bool cGameEffectManager::PlayAreaExplosion(const DirectX::XMFLOAT2& position, float radius)
{
	static constexpr float WARM_EXPLOSION_VISIBLE_DIAMETER_RATIO = 20.0f / 32.0f;
	...
	const float scale = radius * 2.0f / (definition->DrawWidth * WARM_EXPLOSION_VISIBLE_DIAMETER_RATIO);
	const DirectX::XMFLOAT2 centered_position = { position.x + definition->PivotOffset.x * scale,
...
{
	Play(GameEffectType::EnemyDefeatSmoke, position, 0.78f, { 0.72f, 0.78f, 0.88f, 0.86f });
}

const cGameEffectManager::Definition* cGameEffectManager::GetDefinition(GameEffectType type) const
{
