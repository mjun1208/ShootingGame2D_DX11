...
{
	if (!m_IsActive || m_Desc.LightRadius <= 0.0f || m_Desc.LightStrength <= 0.0f)
	{
		return false;
	}

	const float duration = m_Desc.FrameTime * static_cast<float>(m_Desc.FrameCount);
	const float progress = duration > 0.0f ? Saturate(m_ElapsedTime / duration) : 1.0f;
	const float attack = Saturate(progress / 0.06f);
	const float fade = 1.0f - Saturate((progress - 0.28f) / 0.72f);
	const float smooth_fade = SmoothStep(fade);
