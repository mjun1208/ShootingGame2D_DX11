...
class cGameEffectManager final
{
  public:
	...
	void Clear();
	void Draw() const;
	int AppendPointLights(SpritePointLight* lights, int light_count, int capacity) const;
	bool Play(GameEffectType type, const DirectX::XMFLOAT2& position, float scale = 1.0f,
	          const DirectX::XMFLOAT4& color = { 1.0f, 1.0f, 1.0f, 1.0f }, float rotation = 0.0f);
	bool PlayAreaExplosion(const DirectX::XMFLOAT2& position, float radius);
  ...
  private:
	...
	static constexpr std::size_t TYPE_COUNT = static_cast<std::size_t>(GameEffectType::Count);
