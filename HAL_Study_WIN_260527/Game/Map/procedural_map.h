...
struct ProceduralMapOverviewLayout
{
	...
	DirectX::XMFLOAT2 PanelOrigin{ 0.0f, 0.0f }; // 시작 방을 중앙에 맞추기 전 패널의 화면 기준 원점.
...
void ProceduralMap_Draw(const DirectX::XMFLOAT2& camera_position, const DirectX::XMFLOAT2& viewport_size);
...
void ProceduralMap_DrawFadeOverlay(const DirectX::XMFLOAT2& viewport_size, float alpha);
ProceduralMapOverviewLayout ProceduralMap_GetOverviewLayout(const DirectX::XMFLOAT2& viewport_size, bool expanded);
ProceduralMapOverviewLayout ProceduralMap_DrawOverview(const DirectX::XMFLOAT2& viewport_size, bool expanded,
...
DirectX::XMFLOAT2 ProceduralMap_ClampCameraPosition(const DirectX::XMFLOAT2& desired_position,
                                                    const DirectX::XMFLOAT2& viewport_size);

bool ProceduralMap_IsCircleWalkable(const DirectX::XMFLOAT2& position, float radius);
bool ProceduralMap_IsSegmentWalkable(const DirectX::XMFLOAT2& start, const DirectX::XMFLOAT2& end, float radius);
DirectX::XMFLOAT2 ProceduralMap_TraceWalkableSegment(const DirectX::XMFLOAT2& start, const DirectX::XMFLOAT2& direction,
                                                     float maximum_distance, float radius, float trace_step = 8.0f);
DirectX::XMFLOAT2 ProceduralMap_MoveActorCircle(const DirectX::XMFLOAT2& position, const DirectX::XMFLOAT2& movement,
                                                float radius);
...
bool ProceduralMap_TryGetRoomSpawnPosition(int room_index, const DirectX::XMFLOAT2& avoid_position,
                                           float minimum_distance, float clearance_radius,
