...
namespace
{
    ...
    bool LoadBinary(const char* path, std::vector<unsigned char>& binary)
    {
        ...
        return static_cast<bool>(stream.read(
            reinterpret_cast<char*>(binary.data()), size));
    }
    ...
    void FillAnimatedConstants(PixelConstants& constants)
    {
        ...
        {
            constants.Radius = g_MaxRadius *
                EaseOutCubic(g_ElapsedTime / WAVE_OUT_DURATION);
        }
        else if (g_Phase == EffectPhase::WaveReturn)
        {
            const float local_time = g_ElapsedTime - WAVE_OUT_DURATION;
            constants.Radius = g_MaxRadius *
                (1.0f - SmoothStep(local_time / WAVE_RETURN_DURATION));
        }
        else if (g_Phase == EffectPhase::Restore)
        {
            const float local_time = g_ElapsedTime - WAVE_OUT_DURATION -
                WAVE_RETURN_DURATION - STOP_HOLD_DURATION;
            constants.Radius = g_MaxRadius *
                (1.0f - SmoothStep(local_time / RESTORE_DURATION));
        }
    }
}
    ...
    unsigned int height)
{
    ...
    if (!LoadBinary("asset/shader/shader_vertex_time_stop.cso", shader_binary) ||
        FAILED(device->CreateVertexShader(
            shader_binary.data(), shader_binary.size(), nullptr, &g_VertexShader)))
    {
        ReleaseResources();
        return false;
    }
    shader_binary.clear();
    if (!LoadBinary("asset/shader/shader_pixel_time_stop.cso", shader_binary) ||
        FAILED(device->CreatePixelShader(
            shader_binary.data(), shader_binary.size(), nullptr, &g_PixelShader)))
    {
        ReleaseResources();
        return false;
    }
...
void TimeStopEffect_EndCapture()
{
    ...
    g_Context->UpdateSubresource(
        g_PixelConstantBuffer, 0, nullptr, &constants, 0, 0);
...
void TimeStopEffect_Trigger(const DirectX::XMFLOAT2& screen_position)
{
    ...
    g_CenterUv.x = std::clamp(
        screen_position.x / static_cast<float>(g_Width), 0.0f, 1.0f);
    g_CenterUv.y = std::clamp(
        screen_position.y / static_cast<float>(g_Height), 0.0f, 1.0f);
    ...
    for (const DirectX::XMFLOAT2& corner : corners)
    {
        ...
        g_MaxRadius = std::max(g_MaxRadius, std::sqrt(dx * dx + dy * dy));
    }
    g_MaxRadius += 0.06f;
    g_ElapsedTime = 0.0f;
...
void TimeStopEffect_Update(float delta_time)
{
    ...
    g_ElapsedTime += std::max(delta_time, 0.0f);
    if (g_ElapsedTime < WAVE_OUT_DURATION)
    {
        g_Phase = EffectPhase::WaveOut;
    }
    else if (g_ElapsedTime < WAVE_OUT_DURATION + WAVE_RETURN_DURATION)
    {
        g_Phase = EffectPhase::WaveReturn;
    }
    else if (g_ElapsedTime < WAVE_OUT_DURATION + WAVE_RETURN_DURATION + STOP_HOLD_DURATION)
    {
