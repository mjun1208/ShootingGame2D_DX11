...
static std::array<cCollisionBody, COLLISION_BODY_MAX> g_CollisionBodies{};
static int g_CollisionBodyCount = 0;

static std::array<cCollisionHit, COLLISION_HIT_MAX> g_CollisionHits{};
...
{
	int BucketHeads[CollisionTuning::BucketCount]{};
...
{
	const int center_x = WorldToGridCell(body.Circle.Center.x, static_cast<float>(CollisionTuning::CellSize));
	const int center_y = WorldToGridCell(body.Circle.Center.y, static_cast<float>(CollisionTuning::CellSize));
	const int search_range = static_cast<int>((body.Circle.Radius + g_CollisionGrid.MaxRadius) /
	                                          static_cast<float>(CollisionTuning::CellSize)) +
	                         1;
...
static void CollisionSystem_AddBodyToGrid(int body_id)
{
	...
	const int cell_x = WorldToGridCell(body.Circle.Center.x, static_cast<float>(CollisionTuning::CellSize));
	const int cell_y = WorldToGridCell(body.Circle.Center.y, static_cast<float>(CollisionTuning::CellSize));
	const int bucket_index = SpatialHashIndex(cell_x, cell_y, CollisionTuning::BucketCount, 73856093u, 19349663u);
...
static void CollisionSystem_ClearGrid()
{
	...
	for (int i = 0; i < CollisionTuning::BucketCount; ++i)
	{
	...
	for (int grid_y = range.MinY; grid_y <= range.MaxY; ++grid_y)
	{
		for (int grid_x = range.MinX; grid_x <= range.MaxX; ++grid_x)
		{
			const int bucket_index =
			    SpatialHashIndex(grid_x, grid_y, CollisionTuning::BucketCount, 73856093u, 19349663u);
			for (int other_id = g_CollisionGrid.BucketHeads[bucket_index]; other_id != COLLISION_INVALID_ID;
			     other_id = g_CollisionGrid.NextBody[other_id])
			{
				if (g_CollisionGrid.BodyCellX[other_id] != grid_x || g_CollisionGrid.BodyCellY[other_id] != grid_y)
				{
					continue;
				}
...
bool Collision_IsHitCircle(const cCircleCollider& a, const cCircleCollider& b)
{
	...
	return DistanceSquared(a.Center, b.Center) <= radius_sum * radius_sum;
}

bool Collision_IsHitCircle(const DirectX::XMFLOAT2& center_a, float radius_a, const DirectX::XMFLOAT2& center_b,
                           float radius_b)
{
	return Collision_IsHitCircle(cCircleCollider{ center_a, radius_a }, cCircleCollider{ center_b, radius_b });
}
...
int CollisionSystem_RegisterCircle(int owner_id, CollisionLayer layer, CollisionLayer hit_mask,
                                   const DirectX::XMFLOAT2& center, float radius, bool is_active)
{
	...
	g_CollisionBodies[body_id] = {
		owner_id, layer, hit_mask, cCircleCollider{ center, clamped_radius }, is_active,
	};
