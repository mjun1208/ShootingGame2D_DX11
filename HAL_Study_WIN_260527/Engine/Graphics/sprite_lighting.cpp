...
void SpriteLighting_SetWorldLighting(float ambient_brightness, const XMFLOAT3& direct_color, float direct_strength,
                                     const SpritePointLight* point_lights, int point_light_count)
{
	...
	g_State.PointLightCount = point_lights ? std::clamp(point_light_count, 0, SPRITE_POINT_LIGHT_CAPACITY) : 0;
	if (g_State.PointLightCount > 0)
	{
		std::copy_n(point_lights, g_State.PointLightCount, g_State.PointLights.begin());
	}
}
...
void SpriteLighting_SetRadialLight(const XMFLOAT2& screen_position, float radius, float ambient_brightness,
                                   float peak_brightness, float gamma, const XMFLOAT3& color, float color_strength)
{
	g_State.RadialLight = {
		screen_position.x,
		screen_position.y,
...
SpriteLightConstants SpriteLighting_BuildConstants(const XMFLOAT4X4& view_matrix, bool lighting_enabled,
                                                   bool radial_light_enabled)
{
	...
	const float view_scale =
	    XMVectorGetX(XMVector3Length(XMVector3TransformNormal(XMVectorSet(1.0f, 0.0f, 0.0f, 0.0f), view)));
	for (int i = 0; i < g_State.PointLightCount; ++i)
	{
		const SpritePointLight& light = g_State.PointLights[i];
		const XMVECTOR world_position = XMVectorSet(light.Position.x, light.Position.y, 0.0f, 1.0f);
