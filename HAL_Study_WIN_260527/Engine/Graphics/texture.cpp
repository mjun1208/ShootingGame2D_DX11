...
// テクスチャ管理用の配列
static std::array<Texture, TEXTURE_MAX> g_Textures{};
...
int Texture_Load(const wchar_t* pFileName, bool bMipMap)
{
	...
	for (int i = 0; i < TEXTURE_MAX; i++)
	{
		...
		HRESULT hr;
		// UNORM 백 버퍼에 직접 출력하므로 WIC로 읽은 이미지는
		// 저장된 화면 표시용 RGB 값을 유지한다. PNG의
		// sRGB/gAMA 정보만 자동 변환하고 렌더 타깃에서 sRGB 인코딩을 하지 않으면
		// 스프라이트가 원본 이미지보다 훨씬 어두워진다.
		hr = CreateWICTextureFromFileEx(g_pDevice, bMipMap ? g_pContext : nullptr, pFileName, 0, D3D11_USAGE_DEFAULT,
		                                D3D11_BIND_SHADER_RESOURCE, 0, 0, WIC_LOADER_IGNORE_SRGB,
