...
// 적과 보스 투사체의 최대 개수를 수용하고 플레이어, 이펙트,
// 포털, 상자, 맵 횃불에 사용할 여유 공간도 확보한다.
...
inline constexpr int SPRITE_POINT_LIGHT_CAPACITY = SPRITE_POINT_LIGHT_CAPACITY_VALUE;
...
void SpriteLighting_SetWorldLighting(float ambient_brightness, const DirectX::XMFLOAT3& direct_color,
                                     float direct_strength, const SpritePointLight* point_lights,
                                     int point_light_count);
void SpriteLighting_DisableWorldLighting();
void SpriteLighting_SetRadialLight(const DirectX::XMFLOAT2& screen_position, float radius, float ambient_brightness,
                                   float peak_brightness, float gamma, const DirectX::XMFLOAT3& color,
                                   float color_strength);
void SpriteLighting_DisableRadialLight();
SpriteLightConstants SpriteLighting_BuildConstants(const DirectX::XMFLOAT4X4& view_matrix, bool lighting_enabled,
                                                   bool radial_light_enabled);

#else

static const int SPRITE_POINT_LIGHT_CAPACITY = SPRITE_POINT_LIGHT_CAPACITY_VALUE;
