/*==============================================================================
   ...
   Direct3D11用 ビットマップテキスト表示 [bitmap_text.h]
														 Author : Youhei Sato
														 Date   : 2025/06/15
--------------------------------------------------------------------------------
...
namespace hal
{
	class BitmapText
	{
	  ...
	  public:
		BitmapText() = delete;
		BitmapText(ID3D11Device* pDevice, ID3D11DeviceContext* pContext, const wchar_t* pFontTextureFileName,
		          UINT screenWidth, UINT screenHeight, float offsetX = 0.0f, float offsetY = 0.0f, ULONG maxLine = 0,
		          ULONG maxCharactersPerLine = 0, float lineSpacing = 0.0f, float characterSpacing = 0.0f);
		~BitmapText();
	...
	std::unique_ptr<BitmapText> CreateCenteredText(
	    const char* text, float center_x, float offset_y, float character_spacing, float glyph_size = 32.0f,
	    const wchar_t* font_path = L"asset/font/fixedsys/FixedsysExcelsior_ascii_320x512.png",
