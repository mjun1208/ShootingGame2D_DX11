...
void Initialize()
{
	...
	{
		const WeaponData& weapon_data = GameDataManager::GetInstance()
			.GetWeaponGameData()
...
int GetTextureID(BulletType type)
{
	...
	return type_index >= 0 && type_index < WEAPON_COUNT ?
		g_TextureIDs[type_index] : TEXTURE_INVALID_ID;
}

cProjectileDesc Build(
	BulletType type,
	...
	float lifetime_multiplier)
{
	...
	const int type_index = static_cast<int>(type);
	desc.TextureID = type_index >= 0 && type_index < WEAPON_COUNT ?
		g_TextureIDs[type_index] : TEXTURE_INVALID_ID;
